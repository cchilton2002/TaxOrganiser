...
def upload_payslip():
    ...
    for file in files:
        ...
        if "holiday_pay" in parsed_data:
            earnings.append(
                {"type": "Holiday Pay", "amount": parsed_data["holiday_pay"]}
            )
        if "service_charge" in parsed_data:
            earnings.append(
                {"type": "Service Charge", "amount": parsed_data["service_charge"]}
            )
            ...
            deductions.append(
                {
                    ...
                    "amount": parsed_data["national_insurance"],
                }
            )
        if "employee_contribution" in parsed_data:
            deductions.append(
                {"type": "Pension", "amount": parsed_data["employee_contribution"]}
            )

        pension_data = [
            {
                "type": "employee_contribution",
                "amount": parsed_data.get("employee_contribution", 0),
            },
            {
                "type": "employer_contribution",
                "amount": parsed_data.get("employer_contribution", 0),
            },
        ]
...
@finance_bp.route("/payslips", methods=["GET"])
def get_payslips():
    user_id = request.args.get("user_id")
    db = DatabaseManager()

    payslips = db.get_payslips(user_id)

    return jsonify(
        {"message": "Payslips returned", "user_id": user_id, "payslips": payslips}
    ), 200
