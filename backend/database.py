...
class DatabaseManager:
    ...
    def update_summary(self, user_id: int, payment_date: int) -> int:
        ...
        tax_year = (
            pay_date.year - 1
            if (pay_date.month < 4 or (pay_date.month == 4 and pay_date.day < 6))
            else pay_date.year
        )
        ...
        if existing:
            ...
            self.cursor.execute(
                query,
                (total_gross, total_taxable, total_tax, total_ni, user_id, tax_year),
            )
        else:
            # Insert
            query = """
            ...
            self.cursor.execute(
                query,
                (user_id, tax_year, total_gross, total_taxable, total_tax, total_ni),
            )
    ...
    def get_payslips(self, user_id: int):
        self.cursor.execute(
            """
            SELECT payment_date, upload_date, payslip_id
            FROM payslips
            WHERE user_id = %s
